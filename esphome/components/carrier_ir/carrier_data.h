...
#include "esphome/components/remote_base/carrier_protocol.h"
...
using remote_base::CarrierData;

class ControlData : public CarrierData {
 public:
  // Default constructor (power: ON, mode: AUTO, fan: AUTO, temp: 25C)
  ControlData() : CarrierData({CARRIER_TYPE_CONTROL, 0x82, 0x48, 0xFF, 0xFF}) {}
  // Copy from Base
  ControlData(const CarrierData &data) : CarrierData(data) {}
...
class FollowMeData : public CarrierData {
 public:
  // Default constructor (temp: 30C, beeper: off)
  FollowMeData() : CarrierData({CARRIER_TYPE_FOLLOW_ME, 0x82, 0x48, 0x7F, 0x1F}) {}
  // Copy from Base
  FollowMeData(const CarrierData &data) : CarrierData(data) {}
...
class SpecialData : public CarrierData {
 public:
  SpecialData(uint8_t code) : CarrierData({CARRIER_TYPE_SPECIAL, code, 0xFF, 0xFF, 0xFF}) {}
