...
static const char *const TAG = "remote.carrier";
...
uint8_t CarrierData::calc_cs_() const {
  uint8_t cs = 0;
...
bool CarrierData::is_compliment(const CarrierData &rhs) const {
  return std::equal(this->data_.begin(), this->data_.end(), rhs.data_.begin(),
                    [](const uint8_t &a, const uint8_t &b) { return a + b == 255; });
}

void CarrierProtocol::encode(RemoteTransmitData *dst, const CarrierData &src) {
...
static bool decode_data(RemoteReceiveData &src, CarrierData &dst) {
  for (unsigned idx = 0; idx < 6; idx++) {
    uint8_t data = 0;
    for (uint8_t mask = 1 << 7; mask; mask >>= 1) {
...
void CarrierProtocol::dump(const CarrierData &data) { ESP_LOGI(TAG, "Received Carrier: %s", data.to_string().c_str()); }
